...
//  Mandala
//
//  Created by R.AMOGH on 24/05/24.
//
...
class ImageSelector:UIControl{
    ...
    // Properties to manage the image
    ...
    var images: [UIImage] = []{
        didSet{
            imageButtons = images.map{ image in
                ...
                //the first parameter (target) specifies the object that should receive the action message when the control event (specified by the for parameter) occurs.
                //          |
                //          |
                //          |
                //          |
    ...
    @objc private func imageButtonTapped(_ sender: UIButton){
        guard let buttonIndex = imageButtons.firstIndex(of: sender)else{
            preconditionFailure("The buttons and the imaages are not parallel")
        }
